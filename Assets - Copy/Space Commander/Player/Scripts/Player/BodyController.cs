    ...
    public class BodyController : MonoBehaviour
    {
        ...
        */
        ...
        public void Init()
        {
            ...
            playerController.EventOnPlayerClassChange += HandleClassChange;
            
        }

        public void AttachToMobileCenterTarget()
        {
            mainHandAnchor.parent = Player.Mobile.CenterAttachmentPoint.instance.transform;
            mainHandAnchor.localPosition = Vector3.zero;
            mainHandAnchor.localRotation = Quaternion.identity;
            controllerRoot.parent = Mobile.CenterAttachmentPoint.instance.weaponAttachPoint;
            controllerRoot.localPosition = Vector3.zero;
            controllerRoot.localRotation = Quaternion.identity;
        }

        public void AttachToMobileSideTarget()
        {
            /*
            switch (IPlayer.playerWeaponHand)
            {
                case GameManager.PlayerWeaponHand.Left:
                    mainHandAnchor.parent = mobileHandAttachPointLeft;
                    mainHandAnchor.localPosition = Vector3.zero;
                    mainHandAnchor.localRotation = Quaternion.identity;
                    controllerRoot.parent = mobileWeaponAttachPointLeft;
                    controllerRoot.localPosition = Vector3.zero;
                    controllerRoot.localRotation = Quaternion.identity;
                    break;
                case GameManager.PlayerWeaponHand.Right:
                    mainHandAnchor.parent = mobileHandAttachPointRight;
                    mainHandAnchor.localPosition = Vector3.zero;
                    mainHandAnchor.localRotation = Quaternion.identity;
                    controllerRoot.parent = mobileWeaponAttachPointRight;
                    controllerRoot.localPosition = Vector3.zero;
                    controllerRoot.localRotation = Quaternion.identity;
                    break;
            }
        ...
        {
            int pTeam = playerController.GetTeam();
            props.SetColor("_EmissionColor", TeamManager.instance.infos[pTeam].color);
