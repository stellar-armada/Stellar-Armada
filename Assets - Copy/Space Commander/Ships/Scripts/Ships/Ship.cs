...
namespace SpaceCommander.Ships
{
    ...
    public class Ship : NetworkBehaviour
    {
        ...
        {
            /*
            PlayerController pc = shipOwner.GetGameObject().GetComponent<IPlayer>();
                
            if (pc == PlayerController.GetLocalPlayer())
            {
                frame.material.SetColor("_BaseColor", Color.green);
            }
            else if (pc.GetTeam() == PlayerController.GetLocalPlayer().GetTeam())
            {
                frame.material.SetColor("_BaseColor", Color.blue);
            } else
            {
                frame.material.SetColor("_BaseColor", Color.red);
            }
            */
        }
