...
namespace InputHandling
{
    public class VRInputModule : BaseInputModule
    {
        ...
        private bool
            m_isButtonPressedChanged =
                false; // true if controller's button was pressed or released during the last frame
        ...
        private PointerEventData GetPointerEventData()
        {
            ...
            // RaycastResult result = m_pointerEventData.pointerCurrentRaycast;

            if (result.isValid)
            {
                Pointer.instance.SetRaycastLine(true, result.worldPosition);

            }
            else
            {
                Pointer.instance.SetRaycastLine(false, Pointer.instance.pointerStartPoint.position);
            ...
            if (VRInputManager.GetControllerActive())
            {
                ...
                {
                    Pointer.instance.SetRaycastLine(true, m_RaycastResultCache[0].worldPosition);
                    m_RaycastResultCache.Clear();
                }
                else
                {
        ...
        private void ProcessPress(PointerEventData eventData)
        {
            ...
            if (MyIsButtonPressedThisFrame())
            {
                ...
                var newPressed =
                    ExecuteEvents.ExecuteHierarchy(currentOverGo, eventData, ExecuteEvents.pointerDownHandler);
