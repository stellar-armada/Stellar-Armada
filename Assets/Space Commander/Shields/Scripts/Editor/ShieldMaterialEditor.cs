...
namespace SpaceCommander
{
    ...
    public class ShieldMaterialEditor : ShaderGUI
    {
        ...
        public void ShaderPropertiesGUI(Material targetMat)
        {
            ...
            ACTIVATION_TYPE_OPTIONS activationType = ACTIVATION_TYPE_OPTIONS.TEXTURE;

            if (keyWords.Contains("ACTIVATION_TYPE_TEXTURE"))
                activationType = ACTIVATION_TYPE_OPTIONS.TEXTURE;
            if (keyWords.Contains("ACTIVATION_TYPE_TEX_UV"))
                activationType = ACTIVATION_TYPE_OPTIONS.TEXTURE_UV;
            if (keyWords.Contains("ACTIVATION_TYPE_UV"))
                activationType = ACTIVATION_TYPE_OPTIONS.UV;
            if (keyWords.Contains("ACTIVATION_TYPE_CUSTOM_TEX"))
                activationType = ACTIVATION_TYPE_OPTIONS.CUSTOM_TEX;
            if (keyWords.Contains("ACTIVATION_TYPE_X"))
                activationType = ACTIVATION_TYPE_OPTIONS.X;
            if (keyWords.Contains("ACTIVATION_TYPE_Y"))
                activationType = ACTIVATION_TYPE_OPTIONS.Y;
            if (keyWords.Contains("ACTIVATION_TYPE_Z"))
                activationType = ACTIVATION_TYPE_OPTIONS.Z;
            if (keyWords.Contains("ACTIVATION_TYPE_CUSTOM_TEX_CALC_UV"))
                activationType = ACTIVATION_TYPE_OPTIONS.CUSTOM_TEX_CALC_UV;
            ...
            {
                BLEND_MODE_OPTIONS blendModeType = BLEND_MODE_OPTIONS.ADDITIVE_BLEND;
                if (targetMat.GetInt("_BlendSrcMode") == (int) UnityEngine.Rendering.BlendMode.SrcAlpha &&
                    ...
                    blendModeType = BLEND_MODE_OPTIONS.ADDITIVE_BLEND;
                if (targetMat.GetInt("_BlendSrcMode") == (int) UnityEngine.Rendering.BlendMode.SrcAlpha &&
                    targetMat.GetInt("_BlendDstMode") == (int) UnityEngine.Rendering.BlendMode.OneMinusSrcAlpha)
                    blendModeType = BLEND_MODE_OPTIONS.ALPHA_BLEND;
                ...
                if (refractionEnabled)
                {
                    ...
                    blendModeType = (BLEND_MODE_OPTIONS) EditorGUILayout.EnumPopup("", blendModeType);

                if (blendModeType == BLEND_MODE_OPTIONS.ADDITIVE_BLEND)
                {
                ...
                else if (blendModeType == BLEND_MODE_OPTIONS.ALPHA_BLEND)
                {
                    targetMat.SetInt("_BlendSrcMode", (int) UnityEngine.Rendering.BlendMode.SrcAlpha);
            ...
            {
                activationType =
                    (ACTIVATION_TYPE_OPTIONS) EditorGUILayout.EnumPopup("Activation Type:", activationType);

                materialEditor.ShaderProperty(_ActivationRimProperty, "Activation Rim Size");
                if (activationType == ACTIVATION_TYPE_OPTIONS.CUSTOM_TEX ||
                    activationType == ACTIVATION_TYPE_OPTIONS.CUSTOM_TEX_CALC_UV)
            ...
            if (EditorGUI.EndChangeCheck())
            {
                ...
                if (activationEnabled)
                {
                    if (activationType == ACTIVATION_TYPE_OPTIONS.TEXTURE)
                        keywords.Add("ACTIVATION_TYPE_TEXTURE");

                    if (activationType == ACTIVATION_TYPE_OPTIONS.UV)
                        keywords.Add("ACTIVATION_TYPE_UV");

                    if (activationType == ACTIVATION_TYPE_OPTIONS.TEXTURE_UV)
                        keywords.Add("ACTIVATION_TYPE_TEX_UV");

                    if (activationType == ACTIVATION_TYPE_OPTIONS.CUSTOM_TEX)
                        keywords.Add("ACTIVATION_TYPE_CUSTOM_TEX");

                    if (activationType == ACTIVATION_TYPE_OPTIONS.X)
                        keywords.Add("ACTIVATION_TYPE_X");

                    if (activationType == ACTIVATION_TYPE_OPTIONS.Y)
                        keywords.Add("ACTIVATION_TYPE_Y");

                    if (activationType == ACTIVATION_TYPE_OPTIONS.Z)
                        keywords.Add("ACTIVATION_TYPE_Z");

                    if (activationType == ACTIVATION_TYPE_OPTIONS.CUSTOM_TEX_CALC_UV)
