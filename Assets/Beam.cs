...
{ 
    ...
    public class Beam : MonoBehaviour, IDamager
    {
        ...
        void OnDespawned()
        {
            ...
            if (FrameTimerID != -1)
            {
                TimeManager.time.RemoveTimer(FrameTimerID);
                FrameTimerID = -1;
            }
        }
        ...
        { 
            ...
            if (Physics.Raycast(ray, out hitPoint, MaxBeamLength, layerMask))
            {
                ...
                owningWeaponSystem.Impact(WeaponType.Sniper, hitPoint.point + hitPoint.normal*fxOffset);
                        ApplyForce(4f);
                ...
                if (ray2D)
                {
                    ...
                    switch (weaponType)
                    {
                        case WeaponType.Sniper:
                            ...
                            owningWeaponSystem.Impact(WeaponType.Sniper, ray2D.point + ray2D.normal*fxOffset);
                            ApplyForce(4f);
                            break;
                    }
        ...
        void Animate()
        {
            if (BeamFrames.Length > 1)
            {
                ...
                // Add timer 
                FrameTimerID = TimeManager.time.AddTimer(FrameStep, BeamFrames.Length - 1, OnFrameStep);
