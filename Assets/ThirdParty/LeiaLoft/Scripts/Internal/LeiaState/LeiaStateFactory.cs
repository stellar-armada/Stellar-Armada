...
namespace LeiaLoft
{
    ...
    public class LeiaStateFactory
    {
        ...
        public ILeiaState GetState(string id)
        {
            ...
            switch (id.ToLower())
            {
                case "basic": return _builder3D.MakeBasic();
                case "hpo": return _builder3D.MakeHPO();
                case "2d": return _builder2D.MakeBasic();
                case "tvo": return _builder3D.MakeTVO();
            }

            if (_customBuilders.ContainsKey(id))
            {
				return ((AbstractLeiaStateBuilder)Activator.CreateInstance(_customBuilders[id], new object[]{_displayConfig})).MakeBasic();
            }

            this.Fatal(String.Format(UnknownState, id));
            throw new NotSupportedException(String.Format(StateNotSupportedException, id));
        }
    }
}