Shader "HologramShader"
{
	Properties
	{
		...
		_ScanSpeed ("Scan Speed", Range(-2.0, 2000.0)) = 1.0
		// Glow
		_GlowTiling ("Glow Tiling", Range(0.01, 10000.0)) = 0.05
		_GlowSpeed ("Glow Speed", Range(-10.0, 1000.0)) = 1.0
		// Glitch
...
ColorMask RGB
		Pass
		{
			...
			{
				half dirVertex = (dot(i.worldVertex, normalize(half4(_Direction.xyz, 1.0))) + 1) / 2;
				...
				fixed4 col =  _MainColor + (glow * 0.35 * _MainColor) + rimColor;
				col.a = _MainColor.a * _Alpha * (scan + rim + glow) * flicker;
