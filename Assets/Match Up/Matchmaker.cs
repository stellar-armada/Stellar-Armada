...
namespace MatchUp
{
    ...
    public class Matchmaker : MonoBehaviour
    {
        ...
        /// If the connection is lost on the host, the current match is destroyed. If the connection is lost
        /// on the client, the current match is left. 
        /// When the connection is lost the Matchmaker will automatically try and reconnect. If you wish to
        /// implement your own reconnect behaviour make sure to set maxReconnectionAttempts to 0 to disable
        /// the default behaviour.
        /// </remarks>
        public Action onLostConnectionToMatchmakingServer = null;
        ...
        void Start()
        {
            ...
            StartCoroutine(FetchExternalIP(externalIPSource));
        ...
        public IEnumerator ConnectToMatchmaker()
        {
            ...
            while (attempts < maxTries && (matchmakingClient == null || !matchmakingClient.Connected))
            {
                ...
                {
                    Debug.LogError(TAG + "Failed to connect to the matchmaking server. Is it running?\n" + e);
                }
                catch (Exception e)
                {
                    Debug.LogError(TAG + "Failed to connect to the matchmaking server.\n" + e);
                }
...
#else
            UnityWebRequest www = UnityWebRequest.Get(ipSource);
        ...
        IEnumerator SendCommandAsync(Message request, Action<bool, Transaction> onResponse = null)
        {
            ...
            {
                Debug.LogError("Lost connection to MatchUp server.");
        ...
        void KeepAlive(object sender, ElapsedEventArgs args)
        {
            try
            {
                lock (socketLock)
