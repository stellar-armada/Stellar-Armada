...
{
    [MenuItem("Pooling/Pool Manager")]
    public static void OpenPoolManager()
    {
        PoolManager manager = GameObject.FindObjectOfType<PoolManager>();
        if (manager != null)
        {
        ...
        else
        {
            ...
            newManager.AddComponent<PoolManager>();
            Selection.activeGameObject = newManager;
        }
    }

    PoolManager menu; // Menu script that we have to attach somewhere 
    List<PoolContainer> pools = new List<PoolContainer>(); //our local pools.  
    public Dictionary<string, PoolContainer> poolsDict = new Dictionary<string, PoolContainer>(); //dicti 
    ...
    private string assetPath = "Assets/Resources/PoolManagerCache/";
    ...
    void LoadPools()
    {
        if (Application.isPlaying)
            return;
        pools.Clear();
        PoolManagerDB myManager = database as PoolManagerDB;
        if (myManager != null)
        {
    ...
    void SaveManager()
    {
        if (Application.isPlaying)
            return;
        PoolManagerDB newManager = ScriptableObject.CreateInstance<PoolManagerDB>();
    ...
    void CreateNewPoolPressed()
    {
        ...
        PoolContainer newCont = new PoolContainer();
    ...
    void CreateNewDatabasePressed()
    {
        if (Application.isPlaying)
            return;
        PoolManagerDB newManager = ScriptableObject.CreateInstance<PoolManagerDB>();
        newManager.pools = new List<PoolContainer>();
    ...
    void LoadDatabase()
    {
        PoolManagerDB newManager = Resources.Load("PoolManagerCache/" + databaseName) as PoolManagerDB;
        if (newManager != null)
        {
            database = newManager;
    ...
    void CheckForArraysData(PoolContainer pool)
    {
        int i;
    ...
    /// </summary>
    /// <param name="pool">Selected pool</param>
    void CheckForArraysLength(PoolContainer pool)
    {
        int i;
    ...
    void SetItemsCount(PoolContainer pool, int count)
    {
        int i;
        if (pool == null)
        {
