    ...
    public class ShipSelectionManager : MonoBehaviour
    {
        ...
        void UpdateSelectionSet(int selectionSet)
        {
            ...
            foreach (ISelectable selectable in currentSelection)
            {
                ...
                uiShips[selectionSet].Single(s => s.entityId == selectable.GetShip().GetEntityId()).gameObject
                    .SetActive(true);
            }
        }
        ...
        public void RemoveSelectableFromSelectionSets(uint entityId)
        {
            ...
            {
                if (selectionSets[i].Exists(s => s.GetShip().GetEntityId() == entityId))
                {
                    var selectable = selectionSets[i].Single(s => s.GetShip().GetEntityId() == entityId);
                        selectionSets[i].Remove(selectable);
                    UpdateSelectionSet(i);
                    Destroy(selectable.GetShip().GetGameObject());
                }
            }
            OnSelectionChanged?.Invoke();
        }
        ...
        {
            var ships = ShipManager.GetEntities().Where(s => s.GetTeam().teamId == PlayerController.localPlayer.teamId && s.GetType() == typeof(Ship));
        ...
        {
            Debug.Log("AddToSelection called");
